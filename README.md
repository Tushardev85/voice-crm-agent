...
1. Create an Artifact Registry repo:
   ...
   ```bash
   export REPOSITORY="python-pipecat"
...
2. **Update the Twilio Webhook**:
   Copy the ngrok URL and update your Twilio phone number webhook URL to `http://<ngrok_url>/agent`.
...
2. **Run the Docker container**:
   ```sh
   docker run -it --rm -p 8080:8080 twilio-chatbot
   ```

## Usage

The webhook URL will direct the call to your FastAPI application, which will handle it accordingly.
To make an inbound call, call your Twilio phone number using your phone.

To make an outbout call, run the following command:

```
twilio api:core:calls:create --from="+18666062479" --to="+10123456789" --url="https://civet-closing-nominally.ngrok-free.app/agent"
```
