...
def build(c):  # noqa: ANN001, ANN201
    ...
    c.run(
        ...
        f"image={REGION}-docker.pkg.dev/{GOOGLE_CLOUD_PROJECT}/python-pipecat/python-pipecat-microservice-template:manual --no-cache"
    )
...
def deploy(c):  # noqa: ANN001, ANN201
    ...
    c.run(
        "gcloud run deploy python-pipecat-service "
        f"--image {REGION}-docker.pkg.dev/{GOOGLE_CLOUD_PROJECT}/python-pipecat/python-pipecat-microservice-template:manual "
        f"--platform managed --region {REGION}"
    )

# staging
@task(pre=[require_project])
def buildStaging(c):  # noqa: ANN001, ANN201
    """Build the service into a container image"""
    c.run(
        f"gcloud builds submit --pack "
        f"image={REGION}-docker.pkg.dev/{GOOGLE_CLOUD_PROJECT}/pipecat-staging/pipecat-staging-template:manual --no-cache"
    )

@task(pre=[require_project])
def deployStaging(c):  # noqa: ANN001, ANN201
    """Deploy the container into Cloud Run (fully managed)"""
    c.run(
        "gcloud run deploy pipecat-staging-service "
        f"--image {REGION}-docker.pkg.dev/{GOOGLE_CLOUD_PROJECT}/pipecat-staging/pipecat-staging-template:manual "
        f"--platform managed --region {REGION}"
    )
# # staging
# @task(pre=[require_project])
...
#     c.run(
#         f"gcloud builds submit --pack "
#         f"image={REGION}-docker.pkg.dev/{GOOGLE_CLOUD_PROJECT}/python-pipecat-staging/python-pipecat-staging-template:manual --no-cache"
#     )
...
#     """Deploy the container into Cloud Run (fully managed)"""
#     c.run(
#         "gcloud run deploy python-pipecat-staging-service "
#         f"--image {REGION}-docker.pkg.dev/{GOOGLE_CLOUD_PROJECT}/python-pipecat-staging/python-pipecat-staging-template:manual "
#         f"--platform managed --region {REGION}"
#     )

# dev
@task(pre=[require_project])
def buildDev(c):  # noqa: ANN001, ANN201
    """Build the service into a container image"""
    c.run(
        f"gcloud builds submit --pack "
        f"image={REGION}-docker.pkg.dev/{GOOGLE_CLOUD_PROJECT}/python-pipecat-dev/python-pipecat-dev-template:manual --no-cache"
    )

@task(pre=[require_project])
def deployDev(c):  # noqa: ANN001, ANN201
    """Deploy the container into Cloud Run (fully managed)"""
    c.run(
        "gcloud run deploy python-pipecat-dev-service "
        f"--image {REGION}-docker.pkg.dev/{GOOGLE_CLOUD_PROJECT}/python-pipecat-dev/python-pipecat-dev-template:manual "
        f"--platform managed --region {REGION}"
    )
